...
class DatabaseManager:  
    ...
    #             # Automatically handle transactions using context manager
    #             try:
    ...
    #                 print(f"Evaluating record: {record_dict} against condition: {conditions}")  # ✅ Fixed line
    #                 if self.evaluate_conditions(record_dict, conditions):
    #                     filtered.append(record_dict)
    #             from pandas import DataFrame
    #             records = DataFrame(filtered)
    ...
    #         # Ensure columns are correctly set after filtering (if not empty)
    #         records.columns = table.columns  # Ensure columns from the table are used
    ...
    #             # Apply aggregation and rename columns
    #             grouped_df = grouped.agg(**agg_ops).reset_index()
    ...
    #                         filtered.append(record_dict)
    #                 grouped_df = pd.DataFrame(filtered)
    ...
    #                     grouped_df = grouped_df.sort_values(by=col, ascending=(direction == "asc"))

    #             print(grouped_df[selected_cols].to_string(index=False))
    #             print(f"Total grouped records: {len(grouped_df)}")
    ...
    #             print(records[columns].to_string(index=False))
    #             print(f"Total records found: {len(records)}")
